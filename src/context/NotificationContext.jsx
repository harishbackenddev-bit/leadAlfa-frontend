...
const STORAGE_PREFIX = "app_notifications_v1";
const MAX_STORED = 100;

function getUserStorageKey(user) {
  if (!user) return null;
  const id = user.id || user.userId || user.sub || user.email || "session";
...
function persistNotifications(storageKey, list) {
  if (!storageKey) return;
  try {
...
export function NotificationProvider({ children }) {
  ...
  const storageKey = useMemo(() => getUserStorageKey(user), [user]);

  const [notifications, setNotifications] = useState(() => {
    cleanLegacyStorage();
    return loadStoredNotifications(storageKey);
  });
  const [, setTick] = useState(0);

  // Sync notifications when active logged in user / storageKey changes
  useEffect(() => {
    cleanLegacyStorage();
    if (storageKey) {
  ...
  const showNotification = ({
    ...
    const newNotification = {
      ...
      read: false,
      meta: meta ? { ...meta, userId: user?.id, userRole: user?.role } : null,
    };

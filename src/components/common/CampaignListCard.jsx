...
/** Budget may be `{ minBudget, maxBudget }` from API or legacy string */
function budgetDisplayLabel(campaign) {
  const { budget, compensationType, giftNameDescription } = campaign;
  const isGift = String(compensationType || "").toLowerCase().includes("gift");
  if (isGift && String(giftNameDescription || "").trim()) {
    return `Gift · ${giftNameDescription.trim()}`;
  }
  if (isGift) return "Gift";

  const min =
    budget?.minBudget != null && String(budget.minBudget).trim() !== ""
      ? String(budget.minBudget).trim()
      : "";
  const max =
    budget?.maxBudget != null && String(budget.maxBudget).trim() !== ""
      ? String(budget.maxBudget).trim()
      : "";

...
const AdminSidebar = ({ isOpen = true, onClose = () => { }, onToggle = () => { } }) => {
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: <img src={contactIcon} alt="Contact Requests" className="w-6 h-6 object-contain" />,
      activeIcon: (
        <img
          src={contactIcon}
    ...
    {
      label: "Call Requests",
      path: "/admin/book-call-requests",
      icon: <img src={reportsIcon} alt="Call Requests" className="w-6 h-6 object-contain" />,
      activeIcon: <img src={reportsIconActive} alt="Call Requests" className="w-6 h-6 object-contain" />,
    },
    {
      label: "User Feedback",
      path: "/admin/user-feedback",
      icon: <img src={reportsIcon} alt="User Feedback" className="w-6 h-6 object-contain" />,
      activeIcon: <img src={reportsIconActive} alt="User Feedback" className="w-6 h-6 object-contain" />,
    },
    {
      label: "Settings",
      path: "/admin/settings",
  ...
  return (
    <>
      <button
        onClick={onToggle}
        className={`fixed top-2 z-50 md:hidden p-1.5 bg-white rounded-md shadow-md transform transition-transform duration-300 ease-in-out ${isOpen ? "left-[272px]" : "left-4"
          }`}
      ...
      <aside
        className={`w-64 h-screen overflow-y-auto bg-white px-4 py-3 border-r border-gray-100 flex flex-col fixed left-0 top-0 z-40 transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"
          } md:translate-x-0`}
      >
        <div className="flex-1">
          ...
          <nav className="space-y-4">
            {navItems.map((item) => (
              ...
              >
                <div
                  className={`flex items-center gap-2 px-4 py-3 rounded-lg transition-colors ${isActive(item.path)
                ...
                >
                  ...
                  <div className="flex-1 flex items-center justify-start gap-3">
                    <span className="font-medium leading-[20px] text-[14px]">{item.label}</span>
                  </div>
                </div>
              </Link>
            ))}

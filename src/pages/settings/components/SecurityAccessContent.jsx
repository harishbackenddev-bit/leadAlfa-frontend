...
export default function SecurityAccessContent() {
  ...
  return (
    ...
    <SettingsPageHeading
      ...
      subtitle="Manage your password, two-step verification and signed-in devices"
    />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-0">
          ...
          <div className="flex-1 flex justify-start sm:justify-end">
            <button
              ...
              className="px-6 sm:px-8 py-2 sm:py-2.5 btn-gradient text-white text-sm font-medium rounded-full hover:bg-blue-700 transition-colors"
            >
              Edit
            </button>
          </div>
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            className={`relative w-15 h-5 rounded-full transition-colors mr-2 ${
              twoStepEnabled ? 'bg-[#DEE9FF]' : ' btn-gradient'
            }`}
          >
            <span
              className={`absolute top-0 w-5 h-5 rounded-full transition-all duration-300 ${
                twoStepEnabled ? 'left-0 btn-gradient' : 'left-10 bg-[#DEE9FF]'
              }`}
            />
          </button>
        </div>

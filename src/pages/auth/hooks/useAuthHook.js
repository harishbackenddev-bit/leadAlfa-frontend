...
export const useAuth = () => {
  ...
  const verifyEmail = async (verificationData) => {
    try {
      ...
      // Persist verified state so downstream guards (VerifyEmail, Login,
      // ProtectedRoute, signup step-resume) stop treating this session as
      // "needs OTP". The backend may also return a fresh token/user on
      // verification — adopt it when present, otherwise just flip the flag.
      const newToken =
        response?.user?.auth_token ||
        response?.token ||
        response?.access_token ||
        response?.data?.user?.auth_token ||
        response?.data?.token ||
        response?.data?.access_token;
      ...
      if (newToken) {
        dispatch(setAuth({ user: verifiedUser, token: newToken }));
      } else {
        dispatch(updateUserRedux(verifiedUser));
      }

...
const hasPendingCreatorConfirmation = () =>
  sessionStorage.getItem(CREATOR_CONFIRMATION_PENDING_KEY) === "1";
...
export const useSignupFlow = () => {
  ...
  const [currentStep, setCurrentStep] = useState(() =>
    hasPendingCreatorConfirmation() ? 6 : 1
  );
  const [isCreatingProfile, setIsCreatingProfile] = useState(() =>
    hasPendingCreatorConfirmation()
  );
  ...
  useEffect(() => {
    ...
    if (!onboardingUser?.role) {
      if (!roleStepLockedRef.current && currentStepRef.current < 3) {
        setCurrentStep(2);
      }
      return undefined;
    }
  ...
  const handleCreateBrandProfile = async (formSnapshot = formData) => {
    try {
      ...
      clearOnboardingProfileCache();
      dispatch(updateUser({ ...user, profile: created }));

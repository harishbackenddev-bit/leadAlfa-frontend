export default function MessageBubble({ message, isOwn, showTime = true }) {
  ...
  return (
    <div className={`mb-3 flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
      {/* Message Content */}
      <div
        className={`max-w-[82%] sm:max-w-[75%] lg:max-w-[520px] ${
          hasMedia
            ? `bg-transparent p-0 ${isOwn ? 'ml-auto' : 'mr-auto'}`
            : isOwn
            ? 'rounded-xl rounded-br-none btn-gradient px-4 py-3 text-white shadow-sm'
            : 'rounded-xl rounded-bl-none border border-gray-100 bg-white px-4 py-3 text-[#0f172a] shadow-sm'
        }`}
      >
      ...
      {showTime && (
        <p className={`mt-1 text-xs text-[#94a3b8] ${isOwn ? 'text-right' : 'text-left'}`}>
          {message.time}
        </p>
      )}
    </div>

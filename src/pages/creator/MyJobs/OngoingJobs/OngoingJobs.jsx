...
import contentVideo1 from "../../../../assets/images/creator/pitchVideo.png";
...
export default function OngoingJobs({
  ...
  const isViewOffers = viewMode === "offers";
  ...
  const jobData = {
    ...
    // Ongoing job specific data
    coverLetter:
      "The brand is actively seeking talented creators who have the ability to produce captivating and high-quality unboxing videos that not only showcase the product in an appealing and authentic manner but also highlight its key features, packaging, and overall user experience. These videos should be visually engaging and emotionally compelling, capturing the audience's attention from the very beginning while effectively communicating the value and uniqueness of the product. The goal is to create content that feels genuine and trustworthy, helping potential customers connect with the brand on a deeper level and making the unboxing process an exciting and shareable moment. Creators are expected to bring their own creative flair and storytelling skills to ensure the videos resonate with the target audience and align with the brand's tone, aesthetics, and overall marketing objectives.",
    content: [
      { id: 1, image: contentVideo1, type: "video" },
      { id: 2, image: contentVideo1, type: "video" },
    ],
    revisedFeedback: [
      {
        id: 1,
  ...
  const breadcrumbs = customBreadcrumbs || [
    ...
    { label: "View Campaigns", link: null },
  ];
  ...
  const handleAddRatings = () => {
    navigate(`/creator/my-jobs/${id}/ratings`);
  };

  const handleAccept = () => {
    // Navigate to the proposal submission (apply) page for this invitation
    navigate(`/creator/invitations/apply/${id}`);
  };
    ...
    <div className="max-w-[1920px] mx-auto min-h-screen bg-white">
            ...
            {isViewOffers ? (
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="main-btn text-white font-medium px-8 sm:px-12 py-3 rounded-full text-base transition-colors whitespace-nowrap"
                >
                  Accept
                </button>
                <button
                  onClick={handleReject}
                  className="sec-btn font-medium px-8 sm:px-12 py-3 rounded-full text-base transition-colors whitespace-nowrap"
                >
                  Reject
                </button>
              </div>
      ...
      <div className="max-w-[1920px] mx-auto">
        <div className="bg-white rounded-xl p-6 sm:p-8">
          <h3 className="text-3xl md:text-4xl font-extrabold font-anton text-gray-900 mb-4">
            Cover Letter
          </h3>
      ...
      {/* Content Section - Shows submitted content videos */}
      <div className="max-w-[1920px] mx-auto">
        <div className="bg-white rounded-xl p-6 sm:p-8">
          <h3 className="text-3xl md:text-4xl font-extrabold font-anton text-gray-900 mb-6">
            Content
          </h3>
          <div className="flex flex-wrap gap-6">
            {jobData.content.map((item) => (
              <div
                key={item.id}
                className="relative w-full max-w-md aspect-[7/4] rounded-xl overflow-hidden bg-gray-100"
              >
                <img
                  src={item.image}
                  alt={`Content ${item.id}`}
      ...
      {/* Revised Feedback Section */}
      <div className="max-w-[1920px] mx-auto pb-8">
        <div className="bg-white rounded-xl p-6 sm:p-8">
          <h3 className="text-3xl md:text-4xl font-extrabold font-anton text-gray-900 mb-8">
            Revised Feedback
          </h3>
          <div className="space-y-8">
            {jobData.revisedFeedback.map((item) => (
              <div
                key={item.id}
                className="flex flex-col sm:flex-row items-center sm:items-center gap-4 sm:gap-8"
              >
                {/* Thumbnail */}
                <div className="relative w-full sm:w-48 max-w-[200px] aspect-square rounded-2xl overflow-hidden bg-gray-100 flex-shrink-0">
                  <img
                    src={item.image}
                ...
                <div className="flex-1">
                  <p className="text-gray-700 text-xs sm:text-sm leading-loose text-center sm:text-left">
                    {item.feedback}
                  </p>
                </div>
              </div>

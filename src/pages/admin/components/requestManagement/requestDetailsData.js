...
import { mapBrandRequestToDetails } from "./brandRequestMappers";

export const ADMIN_CREATOR_REQUESTS_CACHE_KEY = "admin_creator_requests_cache";
export const ADMIN_BRAND_REQUESTS_CACHE_KEY = "admin_brand_requests_cache";
...
const requestDetailsById = {
  "req-creator-1": {
    ...
    status: "pending",
    basicInfo: [{ label: "Full Name", value: "Sarah Johnson" }],
    locationDetails: [
...
export const getRequestDetailsByType = (requestId, requestType = "creator") => {
  const cacheKey =
    requestType === "brand"
      ? ADMIN_BRAND_REQUESTS_CACHE_KEY
      : ADMIN_CREATOR_REQUESTS_CACHE_KEY;
  const mapFn =
    requestType === "brand" ? mapBrandRequestToDetails : mapCreatorRequestToDetails;
  ...
  const fallback = getRequestDetails(requestId);
  if (fallback && requestType === "brand" && fallback.type !== "brand") {
    return null;
  }

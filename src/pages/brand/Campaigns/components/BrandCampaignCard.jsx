...
const SUBMISSION_STATUS_ITEMS = [
  { key: "pendingReviews", label: "Under Review", tone: "text-amber-600" },
  { key: "approved", label: "Approved", tone: "text-green-600" },
  { key: "revisionRequested", label: "Revision", tone: "text-blue-600" },
  { key: "rejectedSubmissions", label: "Rejected", tone: "text-red-600" },
];
...
export default function BrandCampaignCard({
  ...
  const totalSubmissions = stats.assetsSubmitted ?? 0;
  const hiredTarget = Number(campaign.raw?.numberOfCreators ?? 0);
  const hiredProgress =
    hiredTarget > 0
      ? Math.min(100, Math.round(((stats.hired ?? 0) / hiredTarget) * 100))
      : null;
    ...
    <article className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <CardActionButton
            icon={Users}
            onClick={() => onViewCreators?.(campaign)}
          >
            View Creators
          </CardActionButton>
          <CardActionButton
            icon={Image}
            onClick={() => onViewAssets?.(campaign)}
          >
            View Assets
          </CardActionButton>
        </div>

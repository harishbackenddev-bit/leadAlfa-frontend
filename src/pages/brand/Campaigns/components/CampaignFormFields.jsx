...
export function DateField({ label, value, onChange }) {
  ...
  return (
    <div>
      ...
      <div
        className="relative h-[48px] rounded-[12px] border border-[#e5e7eb] bg-white px-4"
        role="button"
        tabIndex={0}
        onClick={openPicker}
      ...
      >
        <span
          className={`pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[16px] leading-none ${
            value ? "text-[#111827]" : "text-[#64748B]"
          }`}
        >
        ...
        <Input
          ...
          className="absolute inset-0 h-full w-full cursor-pointer border-0 bg-transparent px-4 text-transparent opacity-0 outline-none focus:outline-none focus:ring-0"
        />
      </div>
    </div>
  );

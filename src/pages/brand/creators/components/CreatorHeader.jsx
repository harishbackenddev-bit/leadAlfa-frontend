...
const StatItem = ({ label, value }) => (
  <div className="min-w-0">
    ...
    <p className="text-[14px] font-semibold text-[#1a1a1a] break-words">
      {value}
    </p>
  </div>
);
    ...
    <div className="px-4 py-5 sm:px-6 sm:py-6">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
        {hasImage ? (
          ...
          <div className="flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-full btn-gradient text-xl font-semibold text-white sm:h-24 sm:w-24">
            {initials || "NA"}
          </div>
        )}
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            <h2 className="text-[22px] font-bold text-[#1a1a1a] sm:text-[24px]">
              {name}
            </h2>
            {verified ? (
            ...
            <div className="mb-3 flex items-center gap-2">
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, index) => (
                  <Star
                    key={index}
                    className={`h-4 w-4 ${showRating && index < Math.round(rating)
          ...
          {bio ? (
            <p className="mb-5 text-[14px] leading-6 text-[#475569]">
              {bio}
            </p>
          ) : null}

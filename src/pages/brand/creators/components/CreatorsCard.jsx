...
function CreatorsCardHeroPreview({ heroImage, introVideoUrl, displayName, initials }) {
  const [imgError, setImgError] = useState(false);

  const isVideoUrl = (url) =>
    /\.(mp4|mov|webm|avi|m4v)(\?.*)?$/i.test(url || "");

  const isVideoHero = isVideoUrl(heroImage) || heroImage === introVideoUrl;
  const videoSrc = introVideoUrl || (isVideoUrl(heroImage) ? heroImage : null);

  if (videoSrc && (isVideoHero || imgError)) {
    return (
      <video
        src={videoSrc}
        preload="metadata"
...
export default function CreatorsCard({
  ...
  const tags = (
    creator?.categories?.length ? creator.categories : creator?.skills || []
  ).slice(0, MAX_TAGS);
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-[#e8edf3] bg-white shadow-[0_8px_24px_rgba(15,23,42,0.06)] transition-shadow hover:shadow-[0_12px_28px_rgba(15,23,42,0.1)] sm:p-3 sm:pb-0 p-2 pb-1">
      <div className="relative aspect-[16/12] w-full overflow-hidden bg-[#dbeafe] sm:aspect-[9/12] rounded-t-xl">
        ...
        >
          {hasAvatar ? (
            ...
            <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full border-2 border-white btn-gradient text-sm font-semibold text-white shadow-md">
              {creator?.initials}
            </div>
          )}
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[13px] text-white/85">{city}</p>
          </div>
        </button>
      </div>
      ...
      <div className="flex flex-1 flex-col sm:pt-4 sm:pb-4">
        ...
        <div className="mb-3 flex min-h-[28px] flex-wrap gap-2">
          {tags.length ? (
            tags.map((tag) => (
              <span
                key={tag}
                className="rounded-full bg-[#E0F2FE] px-3 py-1 text-[12px] font-medium text-[#0284C7]"
              >
                {tag}
              </span>
            ))
        ...
        <p className="mb-5 line-clamp-2 flex-1 text-[13px] leading-5 text-[#475569]">
        ...
        <Button
          ...
          className="h-12 w-full rounded-xl btn-gradient text-[14px] font-semibold hover:bg-[#1a54c4]"
        >
          Invite to Campaign
        </Button>
      </div>

...
const TagGroup = ({ title, items, variant = "blue" }) => {
  ...
  return (
    <div>
      <h3 className="mb-3 font-['Manrope:SemiBold',sans-serif] text-[15px] font-semibold text-[#1a1a1a]">
        {title}
      </h3>
      <div className="flex flex-wrap gap-2">
        {items.map((item) => (
          <span
            key={`${title}-${item}`}
            className={`rounded-full px-3 py-1.5 font-['Manrope:Medium',sans-serif] text-[12px] font-medium ${tagClass}`}
          >
            {item}
          </span>
        ))}

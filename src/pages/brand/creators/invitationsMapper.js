...
/**
 * Format compensation for table display.
 *  - "Cash" → "R 1,000.00 - R 3,000.00" (or single value if only one is set)
 *  - "Gift" → giftNameDescription
 *  - Anything else → "—"
 */
export const formatCompensation = (campaign = {}) => {
  if (!campaign) return "—";
  ...
  const min = formatZAR(campaign.minBudget);
  const max = formatZAR(campaign.maxBudget);
